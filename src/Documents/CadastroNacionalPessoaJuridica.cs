...
/// </summary>
public static class CadastroNacionalPessoaJuridica
{
    ...
    {
        var sum = 0;
        for (var i = 1; i < Multiplier.Length; i++) sum += cleanedCnpj[i - 1].ToDigitRepresentation(offset: 48) * Multiplier[i];
    ...
    {
        var sum = 0;
        for (var i = 0; i < Multiplier.Length; i++) sum += cleanedCnpj[i].ToDigitRepresentation(offset: 48) * Multiplier[i];
        return CalculateDigit(sum);
    }
